...
public class BqssApi {
    public static final String API_BASE = "http://open-api.biaoqingmm.com/open-api";

    public static void accessGetApi(String apiName, Map<String, String> params, NetworkManager.ResultCallback callback) {
        try {
            completeParams(API_BASE + apiName, params);
            NetworkManager.get(new BaseNetworkTask(API_BASE + apiName,params,null,callback));
        } catch (NoSuchAlgorithmException e) {
            e.printStackTrace();
        }
    }

    public static void accessPostApi(String apiName, Map<String, String> params, byte[] data, NetworkManager.ResultCallback callback) {
        try {
            completeParams(API_BASE + apiName, params);
            NetworkManager.get(new BaseNetworkTask(API_BASE + apiName,params,data,callback));
        } catch (NoSuchAlgorithmException e) {
            e.printStackTrace();
        }
    }
